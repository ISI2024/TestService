...
# USER
async def create_user(db_session: Session, user_data: dict):
...
async def get_all_users(db_session: Session):
    return db_session.query(Users).all()


async def update_user(db_session: Session, user_login: str, update_data: dict):
...
async def update_wallet(db_session: Session, user_login: str, change: float):
    current_data = db_session.query(Users).filter(Users.login == user_login).first()
    db_session.query(Users).filter(Users.login == user_login).update({'wallet': current_data.wallet + change})
    db_session.commit()


async def delete_user(db_session: Session, login: str):
